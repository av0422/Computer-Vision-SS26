...
for(i=1; i<=nx; i++) {
    for(j=1; j<= ny; j++) {
	 ...
	 /* time derivative with forward difference */
	 df_dz = (f2[i][j] - f1[i][j]) / ht;
...
float disc;       /* discriminant for eigenvalue computation */
float lambda1;    /* larger eigenvalue */
float lambda2;    /* smaller eigenvalue */
float denom;      /* denominator for normal flow */
...
for(i=1; i<=nx; i++) {
	...
	if (lambda1 < eps) {
	    ...
	    /* normal flow only */
	    denom = trace * trace;
	    u[i][j] = -(dxx[i][j] * dxz[i][j] + dxy[i][j] * dyz[i][j]) / denom;
	    v[i][j] = -(dxy[i][j] * dxz[i][j] + dyy[i][j] * dyz[i][j]) / denom;
	    c[i][j] = 128.0;
	} else {
	    /* full flow - Cramer's rule */
	    u[i][j] = (-dxz[i][j] * dyy[i][j] + dyz[i][j] * dxy[i][j]) / det;
	    v[i][j] = ( dxz[i][j] * dxy[i][j] - dyz[i][j] * dxx[i][j]) / det;
	    c[i][j] = 255.0;
	}
